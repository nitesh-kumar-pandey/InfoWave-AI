...
from pypdf import PdfReader          # ✅ Fast direct PDF reading (no subprocess overhead)
...
# ✅ Tuned chunk sizes: smaller = more precise retrieval, faster embedding
CHUNK_SIZE      = 1200
CHUNK_OVERLAP   = 120

TOP_K           = 8   # Retrieve more for better coverage in targeted mode

# ✅ MapReduce limits — kept low to avoid rate limits; increase if on paid tier
MAX_MAP_WORKERS = 4   # Groq free tier: ~30 RPM, don't hammer it
MAX_MAP_CHUNKS  = 40  # Cap chunks processed in full-read mode

# ✅ FAISS disk cache
FAISS_CACHE_DIR = Path(".faiss_cache")
FAISS_CACHE_DIR.mkdir(exist_ok=True)

# ✅ Embedding batch/parallel settings
EMBED_BATCH_SIZE  = 64   # Larger batches = fewer round-trips
EMBED_MAX_WORKERS = 1

# ✅ Large PDF threshold: switch strategy above this many chunks
...
# ─────────────────────────────────────────────
#  CUSTOM CSS  (identical to original)
# ─────────────────────────────────────────────
st.markdown(
    """
    <style>
    ...
    .toggle-pill.doc-active {
        border-color: var(--doc-accent);
        background: rgba(232,112,74,0.08);
        color: var(--doc-accent);
        box-shadow: 0 0 20px rgba(232,112,74,0.12);
    }
    .toggle-pill.web-active {
        border-color: var(--web-accent);
        background: rgba(74,200,232,0.08);
        color: var(--web-accent);
        box-shadow: 0 0 20px rgba(74,200,232,0.12);
    }
    ...
    .metrics-row { margin-top: 1.2rem; display: flex; flex-wrap: wrap; gap: 0.5rem; }
    .metric-pill {
    ...
    .metric-pill b { color: var(--gold); font-weight: 500; }
...
def get_embeddings(model: str) -> HuggingFaceEmbeddings:
    ...
    Models are downloaded once and cached locally by the HuggingFace hub.
    Recommended models:
      ...
      - "BAAI/bge-small-en-v1.5"                    (fast + strong, 384-dim)
...
#  HuggingFace embeddings are thread-safe but
#  batching is still the most efficient approach.
# ─────────────────────────────────────────────
def embed_documents_fast(
    ...
    """
    Embeds texts in sequential batches with an optional progress callback.
    HuggingFace models run locally — no API rate limits to worry about.
    """
...
# ✅ Hierarchical summary prompt for very large PDFs
...
# ─────────────────────────────────────────────
#  ✅ FAST PDF LOADER using pypdf directly
#  (Avoids PyPDFDirectoryLoader overhead + temp dir I/O)
# ─────────────────────────────────────────────
def load_pdfs_fast(uploaded_files: list) -> list[Document]:
    """
    Read PDFs directly from UploadedFile buffers using pypdf.
    No temp directory needed. Much faster than PyPDFDirectoryLoader.
    """
    docs = []
...
# ─────────────────────────────────────────────
#  ✅ SMART SAMPLING: uniform + score-weighted
# ─────────────────────────────────────────────
def smart_sample_docs(all_docs: list, question: str, vectors, k: int) -> list[Document]:
    """
    For large PDFs, pick a smarter subset than simple uniform sampling:
    - Half the budget: top-k similarity hits (relevance)
    - Half the budget: uniform coverage (don't miss anything)
    Deduplicates by index.
    """
    half = k // 2

    # Similarity-based half
    try:
...
# ─────────────────────────────────────────────
#  ✅ IMPROVED MAPREDUCE with hierarchical reduce
#     for very large PDFs
# ─────────────────────────────────────────────
def ask_full_mapreduce(question: str, all_docs: list, llm, vectors=None) -> dict:
    t0 = time.time()

    # ✅ Smart sampling beats blind uniform sampling
    if len(all_docs) > MAX_MAP_CHUNKS:
        if vectors is not None:
            docs_to_process = smart_sample_docs(all_docs, question, vectors, MAX_MAP_CHUNKS)
        else:
            step = len(all_docs) / MAX_MAP_CHUNKS
            docs_to_process = [all_docs[int(i * step)] for i in range(MAX_MAP_CHUNKS)]
    else:
    ...
    # ✅ Status bar so user sees progress
    ...
    with ThreadPoolExecutor(max_workers=MAX_MAP_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            done_count += 1
            status.update(label=f"🔍 Scanned {done_count}/{len(docs_to_process)} sections… ({len(raw_results)} relevant)")
    ...
    if not map_results:
        ...
        # ✅ Hierarchical reduce for very long combined text
        # Split into batches of ~10 extracts, reduce each, then final reduce
        REDUCE_BATCH = 12
        if len(map_results) > REDUCE_BATCH:
            ...
            if partial_answers:
                ...
                result = LLMChain(llm=llm, prompt=HIERARCHICAL_REDUCE_PROMPT).invoke(
                    {"partial_summaries": final_combined, "question": question}
                )
            ...
            result = LLMChain(llm=llm, prompt=REDUCE_PROMPT).invoke(
                {"doc_summaries": combined, "question": question}
            )
            final = result.get("text", "").strip()
...
# ─────────────────────────────────────────────
#  TARGETED RETRIEVAL (top-k similarity)
# ─────────────────────────────────────────────
def ask_targeted(question: str, vectors, llm) -> dict:
    t0              = time.time()
    document_chain  = create_stuff_documents_chain(llm, RAG_PROMPT)
    retriever       = vectors.as_retriever(
        search_type="mmr",          # ✅ MMR reduces duplicate passages
...
with st.sidebar:
    ...
    # ✅ HuggingFace embedding model options — no Ollama required
    embedding_choice = st.selectbox(
        ...
        index=0,
        help="Models are downloaded from HuggingFace Hub and cached locally. No Ollama needed.",
    )
    ...
    st.markdown(
        ...
        f'color:#3a4060;margin-top:0.5rem;line-height:1.6;">'
        f'</p>',
        unsafe_allow_html=True,
    )
    ...
    if session_ids:
        labels = [
            st.session_state.sessions[sid].get("label", sid)
...
if active_session and active_session.get("last_answer"):
    ...
    st.markdown(
        ...
        f'<span style="color:{"#4ac8e8" if src_is_url else "#e8704a"}">{src_text[:80]}{"…" if len(src_text) > 80 else ""}</span>'
        f'{cache_badge}'
        f'</div>',
        unsafe_allow_html=True,
    )
